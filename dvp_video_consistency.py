...
with tf.compat.v1.variable_scope(ARGS.input+'input'):
    input_i = tf.compat.v1.placeholder(tf.float32, shape=[None,None,None,3])
    input_target = tf.compat.v1.placeholder(tf.float32, shape=[None,None,None,3])
lossDict = {}
objDict={} 
with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope()):
    frame_input = input_i[:, :, :, 0:3]
    frame_processed = input_target[:, :, :, 0:3]
    with tf.compat.v1.variable_scope(ARGS.input+'individual'):
        if not with_IRT:
            ...
            tf.compat.v1.summary.image("minor mode", tf.clip_by_value(frame_out_minor, 0, 1))

        tf.compat.v1.summary.image("main mode", tf.clip_by_value(frame_out, 0, 1))        
    with tf.compat.v1.variable_scope(ARGS.input+'loss'):
        if not with_IRT:
            # lossDict["total"] = perceptual_loss(frame_out, frame_processed)
            # lossDict["total"] = tf.reduce_mean(tf.abs(frame_out - frame_processed))
            if ARGS.loss == "l2": 
                lossDict["total"] = tf.reduce_mean(input_tensor=tf.square(frame_out - frame_processed))
            elif ARGS.loss == "l1": 
                lossDict["total"] = tf.reduce_mean(input_tensor=tf.abs(frame_out - frame_processed))
            else:
                lossDict["total"] = perceptual_loss(frame_out, frame_processed)
        else:
            diff_map = tf.reduce_max(input_tensor=tf.abs(frame_out - frame_processed) / (frame_input + 1e-1), axis=3, keepdims=True)
            diff_map1 = tf.reduce_max(input_tensor=tf.abs(frame_out_minor - frame_processed) / (frame_input + 1e-1), axis=3, keepdims=True)
            confidence_map = tf.tile(tf.cast(tf.less(diff_map, diff_map1), tf.float32), [1,1,1,3])
            lossDict["total"] = tf.reduce_mean(input_tensor=tf.abs(frame_out - frame_processed)) + 0.9 * tf.reduce_mean(input_tensor=tf.abs(frame_out_minor - frame_processed)) 
            lossDict["confidence_map"] = tf.reduce_mean(input_tensor=tf.abs(frame_out - frame_processed) * confidence_map) + tf.reduce_mean(
                                            input_tensor=tf.abs(frame_out_minor - frame_processed) * (-confidence_map + 1.0))
        tf.compat.v1.summary.scalar('loss', lossDict["total"])
...
if with_IRT:
    opt_IRT=tf.compat.v1.train.AdamOptimizer(learning_rate=0.0001).minimize(lossDict["confidence_map"], var_list=[var for var in tf.compat.v1.trainable_variables()])
print([var for var in tf.compat.v1.trainable_variables() if var.name.startswith('VCRN')])

merged = tf.compat.v1.summary.merge_all()
writer=tf.compat.v1.summary.FileWriter('./logs/{}'.format(task + '/' + ARGS.output), sess.graph)
...
for epoch in range(1,maxepoch):
    ...
    for id in range(num_of_sample): #range(80):#(min(len(input_names), len(processed_names))):
        ...
        if step % 10 == 0:
            ...
            tf.compat.v2.summary.scalar(name='loss', data=lossDict['total'], step=tf.compat.v1.train.get_or_create_global_step())
        if step % 50 == 0 :
