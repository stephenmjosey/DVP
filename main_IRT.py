...
with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope()):
    ...
    with tf.compat.v1.variable_scope('individual'):
        if not with_IRT:
            ...
            lossDict["confidence_map"] = tf.reduce_mean(input_tensor=tf.abs(frame_out - frame_processed) * confidence_map) + tf.reduce_mean(input_tensor=tf.abs(frame_out_minor - frame_processed) * (-confidence_map + 1.0))
